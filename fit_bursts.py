...
if __name__ == '__main__':
    ...
    for i in range(len(pulses_arr)):
        ...
        for j in range(len(current_burst_id)):
            amp_guesses=np.append(amp_guesses,pulses.loc[indices[current_burst_id[j]],'Amp Guess'])
            time_guesses=np.append(time_guesses,pulses.loc[indices[current_burst_id[j]],'Peak Time Guess'])
        ...
        waterfall,t,peak_bin=import_fil_fits.fits_to_np(filename,dm=dm,maskfile=maskfile,bandpass=True,offpulse=offpulsefile,AO=True,smooth_val=smooth,hdf5=orig_in_hdf5_file,index=pulses_arr[i], tavg=tavg)
        ...
        for l in range(100):
            model=fitter.gen_Gauss2D_model(guess[0],guess[1], f0=guess[2], bw=guess[3], dt=guess[4] )
            bestfit,fitLM=fitter.fit_Gauss2D_model(waterfall, timebins, freqbins, model)
            bestfit_params,bestfit_errors=fitter.report_Gauss_parameters(bestfit,fitLM, verbose=True)
            fitter.plot_burst_windows(timebins, freqbins, waterfall, bestfit, ncontour=8, res_plot=True) #diagnostic plots

            answer=raw_input("Are you happy with the fit y/n?")                                                                                                       
            if answer == 'y':                                                                                        
                break                                                                                                                                                 
            if answer == 'n':                                                                                                                                    
                secondanswer=raw_input("Give the intial guesses in the form t_sb1,t_sb2,...,t_width,f_peak,f_width")                                              
                guessvals = [int(x.strip()) for x in secondanswer.split(',')]                                          
                guess=[list([guessvals[j] for j in range(len(current_burst_id))]),amp_guesses,guessvals[len(current_burst_id)+1],guessvals[int(len(current_burst_id)+2)],guessvals[len(current_burst_id)]]
    ...
    pulses = pd.read_hdf(in_hdf5_file,'pulses')
    fit_numbers = {'gaus amp':amplitudes,'t_cent [s]':t_cent,'t_cent_e [s]':t_cent_e,'t_width [s]':t_width,'t_width_e [s]':t_width_e,'f_cent [MHz]':f_cent,'f_cent_e [MHz]':f_cent_e,'f_width [MHz]':f_width,'f_width_e [MHz]':f_width_e,'f_ref [MHz]':f_ref,'gaus angle':gaus_angle}
