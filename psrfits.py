...
class PsrfitsFile(object):
    ...
    def read_subint(self, isub, apply_weights=True, apply_scales=True, \
        ...
        else: data = data.reshape((self.nsamp_per_subint, self.nchan))
        ...
        if self.npoln > 1:
            data = data.reshape((self.nsamp_per_subint, self.npoln, self.nchan))
            if (self.poln_order=="AABBCRCI" or self.poln_order=="AABB"):
                data = 0.5*(data[:,0,:] + data[:,1,:])
            elif self.poln_order=="IQUV":
                data = data[:,0,:]
        else:
    ...
    def get_spectra(self, startsamp, N):
        ...
        trunc = ((endsub)*self.nsamp_per_subint) - (startsamp+N)

        # Read data
        data = []

        for isub in xrange(startsub, endsub):
