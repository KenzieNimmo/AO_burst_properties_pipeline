...
def barycorr(obs_start, burst_time, f_ref, dm, FRB='R1', telescope='Eff'):
    ...
    if FRB == 'R1':
        FRB_coord=coord.SkyCoord("05:31:58.698", "+33:08:52.586",unit=(u.hourangle, u.deg), frame='icrs')
    ...
    if telescope == 'Arecibo':
        telescope_coord = coord.EarthLocation.from_geodetic(lon = -(66. +45./60. + 11.1/3600.)*u.deg,lat=(18. + 20./60.+36.6/3600.)*u.deg,height=497.*u.m) #arecibo geodetic coords in deg
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(usage='%prog infile_basename', \
                description="Correcting the peak time of the burst to the Solar System Barycentre, corrected to infinite frequency")
    ...
    for i in range(len(pulses_arr)):
        ...
        #reference Frequency
        ref_freq= pulses.loc[pulses_arr[i],'f_ref [MHz]']#using the top of the band as reference (same as PRESTO and spectra.dedisperse from psrfits.py)
